 ...
 * @return sslam::Frame
 */
shared_ptr<Frame> Dataset::GetNextFrame(int idx) {
...
//  cv::imwrite(ss.str(), resized_left);
  ...
  std::ifstream fin;
  if (idx != -1){
    fin = std::ifstream ((ext_m_fmt % dataset_dir % idx).str());
  } else {
    fin = std::ifstream ((ext_m_fmt % dataset_dir % cur_img_index).str());
  }
  if (!fin) {
    return nullptr;
  }
