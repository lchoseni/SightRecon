...
namespace sslam {
    ...
    Camera::Camera(double fx, double fy, double cx, double cy, SE3 pose)
    ...
    Vec3 Camera::World2Camera(const Vec3 &world, const SE3 &T_w_c) {
    ...
    Vec2 Camera::World2Pixel(const Vec3 &world, const SE3 &T_w_c) {
    ...
    Vec3 Camera::Camera2World(const Vec3 &camera, const SE3 &T_c_w) {
        return T_c_w.inverse() * pose_.inverse() * camera;
    }

    Vec3 Camera::Pixel2World(const Vec2 &pixel, const SE3 &T_c_w, double depth) {
